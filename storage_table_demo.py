...
#Display Modified by Kavi
print('Modified by: Kavi\n')
...
resourcegroup_name = 'kavi'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'kavi'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'canadaeast'
...
# Here, we add an entry for first car with four pieces of data - make, model, year, color and price.
#
...
print('Created entry for Honda Civic 2000...')
...
car.price = 30000
table_service.insert_entity('itemstable', car)
print('Created entry for Toyota Camry 2016...')
...
car.price = 55000
table_service.insert_entity('itemstable', car)
print('Created entry for Acura ILX 2017...')
...
# Each entry in a Table is called an 'Entity'. 
# Here, we add an entry for first coffee with four pieces of data - brand, flavor, size of the cup and price per cup.
#
# A partition key tracks how like-minded entries in the Table are created and queried.
# A row key is a unique ID for each entity in the partition
# These two properties are used as a primary key to index the Table. This makes queries much quicker.
...
coffee = Entity()
coffee.PartitionKey = 'coffeelist'
coffee.RowKey = '007'
coffee.brand = 'Folgers'
coffee.flavor = 'Vanilla'
coffee.sizeofcup = 'Small'
coffee.pricepercup = 1
table_service.insert_entity('itemstable', coffee)
print('Created entry for Folgers Vanilla Small...')
...
coffee.flavor = 'Mocha'
coffee.sizeofcup = 'Large'
coffee.pricepercup = 4
table_service.insert_entity('itemstable', coffee)
print('Created entry for Starbucks Mocha Large...')
...
coffee.sizeofcup = 'Medium'
coffee.pricepercup = 3
table_service.insert_entity('itemstable', coffee)
print('Created entry for Tim Hortons Hazelnut Medium...')
